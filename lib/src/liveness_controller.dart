...
class LivenessController extends ChangeNotifier {
      ...
      if (_config.enableTFLite) {
        try {
          ...
          if (!loaded) {
            ...
            _tfliteWarning =
                'TFLite model failed to load (cache cleared — restart to re-download)';
            _tflite = null;
          }
        } catch (e) {
          debugPrint(
      ...
      if (rawForTflite == null) {
        ...
        _tfliteFuture = _tflite!
          ...
          if (tfliteResult != null) {
            ...
            _tfliteWarning =
                'TFLite inference failed — see console for [TFLiteService] error';
            notifyListeners();
          }
          _tfliteFuture = null;
        });
  ...
  Future<void> _onEngineComplete(LivenessResult result) async {
    ...
    if (finalResult.isSuccess) {
      finalResult = await _attachSuccessCapture(finalResult);
  ...
  Future<LivenessResult> _attachSuccessCapture(LivenessResult result) async {
    if (!_config.captureImageOnSuccess) return result;

    final captured = await _cameraService.captureStillImage();
    if (captured == null) return result;

    var path = captured.path;

    if (_config.persistCapturedImage) {
      final persistedPath =
          await _persistCapturedImage(path, sessionId: result.sessionId);
      if (persistedPath != null) path = persistedPath;
    }
    ...
    return result.withCapturedImage(image);
  }

  Future<String?> _persistCapturedImage(String sourcePath,
      {String? sessionId}) async {
    try {
      final appDir = await getApplicationDocumentsDirectory();
      final captureDir =
          Directory('${appDir.path}/${_config.captureDirectoryName}');
      if (!await captureDir.exists()) {
        await captureDir.create(recursive: true);
      }

      final suffix = sessionId?.replaceAll(RegExp(r'[^a-zA-Z0-9_-]'), '_') ??
          DateTime.now().millisecondsSinceEpoch.toString();
      final targetPath = '${captureDir.path}/capture_$suffix.jpg';
      ...
      await _pruneCapturedImages(captureDir);
      return targetPath;
    } catch (e) {
      debugPrint('[LivenessController] Persist capture failed: $e');
      return null;
    }
  }

  Future<void> _pruneCapturedImages(Directory captureDir) async {
    final maxCount = _config.maxStoredCapturedImages;
    if (maxCount <= 0) return;

    try {
      final files = <File>[];
      await for (final entity in captureDir.list(followLinks: false)) {
      ...
      for (final file in files) {
        try {
          ...
          stats.add((file: file, modified: stat.modified));
        } catch (_) {
          // ignore unreadable files during pruning
        }
      }

      stats.sort((a, b) => a.modified.compareTo(b.modified));
      final overflow = stats.length - maxCount;
      for (var i = 0; i < overflow; i++) {
        try {
          await stats[i].file.delete();
        } catch (_) {
          // ignore failed deletions to keep session success path resilient
        }
      }
    } catch (e) {
      debugPrint('[LivenessController] Prune captures failed: $e');
    }
  }
