...
class LivenessConfig {
  const LivenessConfig({
    ...
    this.captureImageOnSuccess = false,
    this.persistCapturedImage = true,
    this.captureDirectoryName = 'flutter_face_liveness_captures',
    this.maxStoredCapturedImages = 30,
    ...
    this.headerBuilder,
    this.footerBuilder,
    this.ovalProgressColor = const Color(0xFF00D4AA),
  });
  ...
  /// Capture a still image when liveness succeeds.
  final bool captureImageOnSuccess;

  /// Move captured images from temporary storage to app documents directory.
  final bool persistCapturedImage;

  /// Directory name under app documents for persisted captures.
  final String captureDirectoryName;

  /// Maximum number of persisted captured images to keep on device.
  /// Older files are deleted first when this limit is exceeded.
  final int maxStoredCapturedImages;
  ...
  /// Optional builder to customize the header section above the oval.
  /// If null, displays the status indicator and step indicator.
  /// Receives context and current [LivenessController] as parameters.
  final Widget Function(BuildContext context)? headerBuilder;

  /// Optional builder to customize the footer section below the oval.
  /// If null, displays the liveness instructions widget.
  /// Receives context and current [LivenessController] as parameters.
  final Widget Function(BuildContext context)? footerBuilder;

  /// Color of the progress ring around the oval.
  /// Default is a cyan color (#00D4AA).
  final Color ovalProgressColor;
  ...
  LivenessConfig copyWith({
    ...
    bool? captureImageOnSuccess,
    bool? persistCapturedImage,
    String? captureDirectoryName,
    int? maxStoredCapturedImages,
    bool? enableFaceMesh,
    bool? enableFaceId,
    ...
    return LivenessConfig(
      ...
      tfliteDeepfakeThreshold:
          tfliteDeepfakeThreshold ?? this.tfliteDeepfakeThreshold,
      enableVideoReplayDetection:
          enableVideoReplayDetection ?? this.enableVideoReplayDetection,
      ...
      captureImageOnSuccess:
          captureImageOnSuccess ?? this.captureImageOnSuccess,
      persistCapturedImage: persistCapturedImage ?? this.persistCapturedImage,
      captureDirectoryName: captureDirectoryName ?? this.captureDirectoryName,
      maxStoredCapturedImages:
          maxStoredCapturedImages ?? this.maxStoredCapturedImages,
      ...
      ovalProgressColor: ovalProgressColor ?? this.ovalProgressColor,
    );
  }
}
