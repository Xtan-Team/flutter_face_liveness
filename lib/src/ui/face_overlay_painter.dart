...
class FaceOverlayPainter extends CustomPainter {
  ...
  void _drawCircularProgress(Canvas canvas, Size size) {
    if (progressValue <= 0.0 || progressValue > 1.0) return;

    final oval = _ovalRect(size);
    // Expand the progress ring slightly outside the oval
    final progressOval = oval.inflate(20);
    
    // Draw the background ring (light version)
    canvas.drawOval(
      progressOval,
      Paint()
    ...
    final sweepAngle = (progressValue * 360) * (math.pi / 180);
    canvas.drawArc(
      progressOval,
  ...
  Rect _ovalRect(Size size) {
    ...
    final cy = size.height / 2;  // Center vertically
  ...
  bool shouldRepaint(FaceOverlayPainter old) =>
      ...
      old.isDark != isDark ||
      old.progressValue != progressValue ||
